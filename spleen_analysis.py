...
def analyze_spleen_results():

    file_path = os.path.dirname(__file__)
    results_dir = os.path.join(file_path, "results")

    for data_to_view in ["spleen-unet", "spleen-unetr", "spleen-atunet"]:
        ...
        print(
            "Dice 96 outside:", mean_outside_dice_96, " sd: (", std_outside_dice_96, ")"
        )
        print(
            "Dice 80 outside:", mean_outside_dice_80, " sd: (", std_outside_dice_80, ")"
        )
        print(
            "Dice 64 outside:", mean_outside_dice_64, " sd: (", std_outside_dice_64, ")"
        )
        print(
            "Dice 48 outside:", mean_outside_dice_48, " sd: (", std_outside_dice_48, ")"
        )
        print(
            "Dice 32 outside:", mean_outside_dice_32, " sd: (", std_outside_dice_32, ")"
        )
        ...
        plt.savefig(
            os.path.join(results_dir, data_to_view + "-dice.png"),
            bbox_inches="tight",
        )
        ...
        locs, labels = plt.yticks()  # Get the current locations and labels.
        plt.yticks(
            np.arange(0, 1.2, step=0.2), labels=["0", "60", "120", "180", "240", "300"]
        )  # Set label locations.
        plt.xlabel("Foreground voxel proportion")
        plt.ylabel("Hausdorff distance of test samples")
        plt.title(data_to_view + "(Spleen)")
        plt.savefig(
            os.path.join(results_dir, data_to_view + "-hausdorff.png"),
            bbox_inches="tight",
        )


if __name__ == "__main__":
    analyze_spleen_results()
